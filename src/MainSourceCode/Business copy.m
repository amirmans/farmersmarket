...
- (void)getProductForBusinessAndCustomer:(NSDictionary *) paramsDict {
    
    [[APIUtility sharedInstance] callServer:paramsDict server:BusinessAndProductionInformationServer method:@"GET" completiedBlock:^(NSDictionary *responseData) {

                if([[responseData valueForKey:@"status"] integerValue] >= 0)
                {
                    if(responseData != nil)
                     {
                         NSError* error =nil;

                         self.businessProducts = responseData;
                         if (error)
                         {
                ...
                {
//                    NSLog(@"Error in fetching product items.  Description of error is: %@", [error localizedDescription]);
                     NSLog(@"Error in fetching product items.");
                      [UIAlertController showErrorAlert: @"Error in fetching product items."];
                }
            }];
}
...
- (void)startLoadingBusinessProductCategoriesAndProducts {
    ...
    NSNumber* nsNumberBusinessID = [NSNumber numberWithInt:businessID];
    NSDictionary *paramsDict = @{ @"cmd":@"products_for_business",@"businessID":nsNumberBusinessID,@"consumerID":consumer_id
        , @"sub_businesses":sub_businesses};
    [self getProductForBusinessAndCustomer:paramsDict];
}
